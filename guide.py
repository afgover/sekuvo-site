...
G["vi"] = dict(
    nav_label="Hướng dẫn",
    title="Sekuvo — Hướng dẫn",
    desc="Cách sử dụng Sekuvo: lần chạy đầu tiên, tự động điền, bàn phím, nhập trên máy tính qua Bluetooth, sao lưu và chuyển giao.",
    h1="Hướng dẫn",
    lede="Mọi thứ Sekuvo làm được, theo thứ tự mà bạn có khả năng cần đến nhất. Không có điều gì ở đây đòi hỏi tài khoản hay kết nối internet.",
    back="← Quay lại sekuvo.com",
    sections=[
        ("start", "Lần chạy đầu tiên", [
            "<p>Khi mở lần đầu, bạn thiết lập một <strong>mật khẩu chính</strong>. Mỗi bí mật bạn lưu được mã hóa bằng khóa dẫn xuất từ nó, vì vậy đây là thứ duy nhất mà Sekuvo không thể giúp bạn khôi phục — hãy ghi lại nó ở nơi an toàn trước khi tiếp tục.</p>",
            "<p>Bạn đang chuyển từ điện thoại khác? Chạm vào <strong>\"Tôi có bản sao lưu — khôi phục nó\"</strong> trên màn hình chào mừng. Bạn vẫn sẽ thiết lập mật khẩu chính cho thiết bị này trước, sau đó bộ chọn tệp sẽ tự động mở ra và các mục của bạn sẽ quay trở lại.</p>",
            "<p>Nếu điện thoại của bạn có cảm biến vân tay hoặc khuôn mặt, Sekuvo sẽ đề nghị sử dụng nó ngay sau khi thiết lập. Đây chỉ là một lớp tiện lợi: mật khẩu chính luôn hoạt động, và nó cũng là thứ tồn tại qua cả việc đặt lại về cài đặt gốc.</p>",
        ]),
        ("entries", "Những gì bạn có thể lưu", [
            "<p>Bốn loại mục bao quát phần lớn những gì mọi người lưu trong một kho lưu trữ:</p>",
            "<ul><li><strong>Tài khoản / Mật khẩu</strong> — tên đăng nhập, mật khẩu, trang web hoặc ứng dụng.</li><li><strong>Hằng ngày</strong> — tên, điện thoại, email, địa chỉ. Những thứ mà các biểu mẫu luôn yêu cầu.</li><li><strong>Thẻ</strong> — số thẻ, ngày hết hạn, CVV, IBAN.</li><li><strong>Ghi chú bảo mật</strong> — văn bản tự do, với các loại phụ dành cho tập lệnh, khóa, mã khôi phục, và tệp cấu hình.</li></ul>",
        ...
        ("autofill", "Tự động điền (bên trong điện thoại)", [
            ...
            "<p><strong>Để bật:</strong> Sekuvo → Cài đặt → <em>Bật tự động điền</em> → chọn Sekuvo từ danh sách hệ thống. Danh sách này có tên khác nhau tùy theo thiết bị: trên Samsung và Android 14+ là <em>Mật khẩu, khóa truy cập &amp; tự động điền → Dịch vụ ưu tiên</em>; trên Android thuần túy là <em>Dịch vụ tự động điền</em>.</p>",
            "<p>Sau đó, chạm vào một trường tên đăng nhập, mật khẩu, hoặc thẻ trong bất kỳ ứng dụng nào sẽ hiển thị các mục khớp của bạn phía trên bàn phím. Nếu kho lưu trữ đang khóa, bước mở khóa sẽ hiện trước — trong khi nó đang khóa, hệ thống hoàn toàn không nhận được bất cứ điều gì.</p>",
        ]),
        ("keyboard", "Bàn phím Sekuvo (bên trong điện thoại)", [
            "<p>Tiện ích bàn phím gõ các giá trị đã lưu trực tiếp vào bất kỳ trường nào, trong bất kỳ ứng dụng nào, không cần bước sao chép.</p>",
            "<p><strong>Để bật:</strong> Sekuvo → Cài đặt → <em>Bật bàn phím</em> → bật nó trong danh sách hệ thống, sau đó chọn \"Bàn phím Sekuvo\" từ bộ chuyển đổi bàn phím bất cứ khi nào bạn cần.</p>",
            "<p>Các mục sử dụng gần đây nằm trên cùng, và 🔍 tìm kiếm trong tiêu đề, tên đăng nhập, và địa chỉ. Trong khi kho lưu trữ đang khóa, bàn phím chỉ hiện các mục được đánh dấu để truy cập nhanh — xem bên dưới.</p>",
        ]),
        ("bluetooth", "Nhập trên máy tính qua Bluetooth", [
            "<p>Điện thoại của bạn có thể hoạt động như một bàn phím Bluetooth và gõ một bí mật vào vị trí con trỏ trên máy tính của bạn. Không có gì được cài đặt trên máy tính, nó hoạt động trên Windows, macOS, và Linux, và giá trị không bao giờ chạm vào bộ nhớ tạm hay mạng. Yêu cầu Android 9 trở lên.</p>",
            "<h3>Từng bước</h3>",
            "<ol><li>Trong Sekuvo, mở mục và chạm vào biểu tượng 💻 cạnh trường bạn muốn gõ. <strong>Giữ màn hình này mở</strong> — điện thoại chỉ tự công bố mình là bàn phím khi màn hình này còn mở.</li><li>Trên máy tính, thêm điện thoại như <em>một thiết bị Bluetooth mới</em>: trên Windows là <em>Cài đặt → Bluetooth &amp; thiết bị → Thêm thiết bị → Bluetooth</em>, trên macOS là <em>Cài đặt Hệ thống → Bluetooth</em>.</li><li>Xác nhận mã ghép nối trên cả hai phía.</li><li>Quay lại điện thoại, chọn máy tính của bạn từ danh sách và chờ chữ \"Đã kết nối\" xuất hiện.</li><li>Chọn bố cục bàn phím của <strong>máy tính</strong> — không phải của điện thoại. Máy tính là bên diễn giải mã phím, nên bố cục sai sẽ âm thầm biến các ký tự như @ \" ? thành ký tự khác.</li><li>Nhấp vào trường trên máy tính nơi giá trị cần đến.</li><li>Chạm <strong>Nhập</strong> trên điện thoại. Sau khi đếm ngược ba giây — được thêm vào để bạn kịp nhấp vào trường đó — giá trị sẽ được gõ từng phím một.</li></ol>",
            "<h3>Đã kết nối nhưng không có gì được nhập</h3>",
            "<p>Hầu như luôn cùng một nguyên nhân: máy tính đã từng ghép nối điện thoại của bạn <em>như một điện thoại thông thường</em> vào một thời điểm trước đó, nên chưa bao giờ bật dịch vụ bàn phím (HID) cho nó. Kết nối vẫn thành công nhưng các lần nhấn phím không đi đến đâu cả.</p>",
            "<ol><li>Gỡ ghép nối trên máy tính (<em>Xóa thiết bị</em>).</li><li>Gỡ nó trên điện thoại nữa (<em>Bỏ ghép nối</em> trong cài đặt Bluetooth). Cả hai phía đều quan trọng.</li><li>Mở màn hình 💻 trong Sekuvo và để nó mở.</li><li>Ghép nối lại, bắt đầu <strong>từ máy tính</strong>.</li><li>Để xác nhận trên Windows: <em>Trình quản lý Thiết bị → Thiết bị Giao diện Người dùng</em> giờ đây phải liệt kê một thiết bị Bluetooth HID.</li></ol>",
            "<p>Trên macOS, kết nối đầu tiên có thể mở <strong>Trình hỗ trợ Thiết lập Bàn phím</strong>, yêu cầu bạn nhấn một phím cạnh phím shift. Cho đến khi cửa sổ đó được đóng lại, macOS không xử lý gì cả — đây là nguyên nhân thường gặp của tình trạng \"đã kết nối nhưng im lặng\" trên máy Mac.</p>",
            "<h3>Tốc độ, và cách kiểm tra nó</h3>",
            "<p>Các lần nhấn phím được gửi từng cái một, nên một bí mật dài thực sự sẽ mất thời gian — hộp thoại hiển thị một ước tính. Có ba tốc độ được cung cấp, và việc An toàn là mặc định là có chủ đích. Trước khi tăng tốc độ, hãy dùng <strong>⏱ Thử nghiệm tốc độ</strong>: nó gõ mười khối giống hệt nhau được phân tách bằng dấu cách và báo cáo số ký tự mỗi giây đo được. Nếu dù chỉ một khối khác biệt, tốc độ đó không an toàn trên máy tính này — hãy giảm xuống một bậc.</p>",
            "<p>Thử nghiệm tốc độ cũng đóng vai trò chẩn đoán: không có gì được nhập nghĩa là vấn đề ghép nối, ký tự bị lộn xộn nghĩa là vấn đề bố cục, việc nhập bị gián đoạn giữa chừng nghĩa là vấn đề tốc độ.</p>",
            "<p>Trong khi nhập, bạn sẽ thấy một thanh tiến trình và một nút <strong>Dừng</strong>. Nếu hàng đợi gửi bị treo, Sekuvo sẽ thử lại với thời gian chờ tăng dần, và nếu vẫn không thành công, nó sẽ <em>dừng lại và cho bạn biết</em> nó dừng ở ký tự nào — không có bí mật nào bị gõ dở dang trong im lặng.</p>",
        ]),
        ("lock", "Cách khóa hoạt động", [
            "<p>Kho lưu trữ khóa lại ngay khoảnh khắc màn hình mà nó đang mở tắt đi — gập điện thoại có thể gập lại cũng được tính. Ngay lúc đó, khóa bị xóa khỏi bộ nhớ; tiêu đề của các mục có thể vẫn còn hiển thị trong danh sách, nhưng không có gì có thể được giải mã.</p>",
            "<p>Rời khỏi ứng dụng trong khi màn hình vẫn bật <em>không</em> khóa nó lại. Đây là điều có chủ đích: ứng dụng, bàn phím, và tự động điền chia sẻ chung một phiên làm việc, nếu không thì việc chuyển sang trình duyệt để dán mật khẩu sẽ khiến bạn bị khóa ngoài giữa chừng công việc.</p>",
        ]),
        ("quick", "Truy cập nhanh — một sự đánh đổi có chủ đích", [
            "<p>Các mục mà bạn đánh dấu rõ ràng là \"dùng trên bàn phím không cần mật khẩu\" được lưu lại lần thứ hai, được mã hóa bằng một khóa thiết bị riêng biệt, để bàn phím có thể đọc chúng <em>ngay cả khi kho lưu trữ đang khóa</em>.</p>",
            "<p>Sự đánh đổi này được nêu rõ: các mục như vậy được bảo vệ bởi khóa màn hình điện thoại của bạn, chứ không phải mật khẩu chính của bạn. Đừng đặt mật khẩu vào đó. Dấu này tắt theo mặc định; ngoại lệ duy nhất là các mục <strong>Hằng ngày</strong> mới, mặc định được đánh dấu vì tên và số điện thoại chính là những gì bạn muốn có sẵn trong tầm tay — bạn có thể tắt bất kỳ mục nào trong số đó.</p>",
        ]),
        ("backup", "Sao lưu và khôi phục", [
            "<p>Sao lưu hệ thống (sao lưu Google, chuyển thiết bị) bị tắt có chủ đích, vì vậy chỉ có đúng một cách để kho lưu trữ của bạn rời khỏi điện thoại, và đó là cách bạn chọn: <strong>Cài đặt → Tạo bản sao lưu mã hóa</strong>.</p>",
            "<p>Bạn chọn một <em>mật khẩu sao lưu</em> — hãy đặt nó khác với mật khẩu chính của bạn, vì mật khẩu sao lưu cuối cùng sẽ được gõ trên các máy tính. Kết quả là một tệp <code>.vaultbak</code> duy nhất mà bạn lưu ở bất cứ đâu bạn muốn: Drive, thẻ SD, ổ USB.</p>",
            "<p>Tệp này độc lập với thiết bị: muối riêng và các tham số dẫn xuất khóa của nó nằm trong phần đầu tệp. Tệp cùng với mật khẩu sao lưu khôi phục mọi thứ trên bất kỳ điện thoại nào, ngay cả sau khi đặt lại về cài đặt gốc. Hãy khôi phục qua <strong>Cài đặt → Khôi phục từ bản sao lưu</strong>, hoặc từ màn hình chào mừng trên một lần cài đặt mới.</p>",
            "<p>Khôi phục cung cấp <em>thêm vào các mục hiện có</em> hoặc <em>thay thế tất cả</em>. Cả hai đều được xác minh trước khi bất cứ điều gì được ghi — mật khẩu sai sẽ thất bại ngay ở bước xác thực, và không có gì bị xóa cho đến khi dữ liệu mới được xác nhận.</p>",
        ]),
        ("transfer", "Mang bí mật từ máy tính về", [
            "<p>Gõ tay một khóa dài trên điện thoại là nơi các lỗi xảy ra. Sekuvo vận chuyển chúng dưới dạng một phong bì mã hóa, theo hai cách: văn bản mà bạn dán, hoặc mã QR mà bạn quét.</p>",
            "<p><strong>Trên điện thoại:</strong> nút ➕ cung cấp <em>Nhập qua QR</em> và <em>Nhập từ văn bản</em>. Nhập qua QR sẽ mở camera; các khung hình được giải mã trên thiết bị, và các lần chuyển giao gồm nhiều khung hình sẽ hiển thị tiến trình khi chúng được thu thập.</p>",
            "<h3>Công cụ phía máy tính</h3>",
            "<p>Công cụ tạo ra các phong bì này là một tệp HTML duy nhất, và nó có chủ đích là <strong>một tệp tải xuống, không phải một trang web</strong>. sekuvo.com sẽ không bao giờ hỏi bạn về một bí mật; một trang làm điều đó sẽ trông y hệt như một trang web lừa đảo.</p>",
            "<ol><li>Mở dự án trên GitHub và vào <strong>Releases</strong>.</li><li>Tải <code>aktar.html</code> từ phiên bản mới nhất.</li><li>Kiểm tra mã SHA-256 của nó với giá trị được công bố kế bên: <code>shasum -a 256 aktar.html</code> trên macOS hoặc Linux, <code>certutil -hashfile aktar.html SHA256</code> trên Windows.</li><li>Mở tệp bằng cách nhấp đúp. Nó chạy từ ổ đĩa của bạn — thanh địa chỉ hiển thị <code>file://</code>, không phải một trang web.</li><li>Dán văn bản của bạn, đặt một mật khẩu chuyển giao, và nó sẽ tạo ra phong bì dưới dạng văn bản hoặc mã QR.</li><li>Trên điện thoại, quét hoặc dán, nhập cùng mật khẩu đó, và chọn thêm hoặc thay thế.</li></ol>",
            "<p>Toàn bộ quá trình mã hóa diễn ra bên trong trình duyệt của bạn, trên máy của bạn. Cũng có <code>vault-clip.py</code> cho dòng lệnh, thực hiện điều tương tự từ bộ nhớ tạm hoặc từ một tệp và có thể vẽ mã QR trong terminal.</p>",
            "<h3>Nếu bạn không muốn dùng công cụ này</h3>",
            "<p>Phong bì là một định dạng mở, không phải thứ chỉ Sekuvo mới tạo được: PBKDF2-HMAC-SHA256 với 310.000 vòng lặp, AES-256-GCM, được bọc trong một đối tượng JSON nhỏ. Bạn có thể tự xây dựng nó với khoảng ba mươi dòng mã mà bạn đã đọc, rồi dán kết quả vào <em>Nhập từ văn bản</em> — ứng dụng không có cách nào biết công cụ nào đã tạo ra nó, và nó cũng không quan tâm.</p>",
            "<p>Công thức nằm trong kho lưu trữ: <a href=\"https://github.com/afgover/Vault/blob/HEAD/docs/kendi-zarfini-uret.md\">docs/kendi-zarfini-uret.md</a> (được viết bằng tiếng Thổ Nhĩ Kỳ).</p>",
        ]),
        ("log", "Nhật ký sử dụng", [
            "<p>Sekuvo ghi lại trường nào của mục nào đã đi đâu và khi nào: đến bộ nhớ tạm, đến máy tính qua Bluetooth (kèm tên thiết bị đích), hoặc được gõ vào một ứng dụng từ bàn phím. Khi một máy tính bị xâm phạm, điều này trả lời câu hỏi \"cái gì đã đến đó, tôi cần thay đổi cái gì\".</p>",
            "<p>Nhật ký được mã hóa bằng cùng khóa với các mục của bạn, và <strong>các giá trị không bao giờ được ghi vào đó</strong> — chỉ có loại sự kiện, tên trường, và đích đến. Xóa một mục cũng xóa nhật ký của nó, và bạn có thể xóa toàn bộ từ Cài đặt.</p>",
        ]),
        ("generator", "Trình tạo mật khẩu", [
            "<p>Có thể truy cập từ biểu tượng 🎲 trên màn hình chính, hoặc cạnh trường mật khẩu khi chỉnh sửa một mục. Nó dùng một nguồn ngẫu nhiên mật mã học, tạo ra từ 8 đến 64 ký tự, cho phép chọn các lớp ký tự, có thể loại bỏ các ký tự giống nhau, và hiển thị độ hỗn loạn (entropy) kết quả tính bằng bit.</p>",
        ]),
    ],
)
...
CAPTIONS = {
    ...
    "vi": {
        "01-karsilama.png": "Màn hình chào mừng: thiết lập mật khẩu chính, hoặc khôi phục một bản sao lưu mà bạn đã có.",
        "02-liste.png": "Danh sách các mục, với tính năng tìm kiếm và bộ lọc theo loại.",
        "10-ekleme-menusu.png": "Khi thêm: bốn loại mục, cùng hai con đường nhập.",
        "04-detay.png": "Một mục. Mật khẩu vẫn được ẩn; các biểu tượng cạnh mỗi trường sẽ sao chép nó hoặc gõ nó trên máy tính.",
        "07-ayarlar.png": "Cài đặt — nơi tự động điền và bàn phím được bật.",
        "11-duzenleme.png": "Đang chỉnh sửa một mục. Công tắc truy cập nhanh nằm bên dưới, chi phí của nó được giải thích ngay cạnh.",
        "08-yedekleme.png": "Sao lưu và khôi phục nằm cạnh nhau trong Cài đặt.",
        "06-aktarim.png": "Trình hướng dẫn mang bí mật từ máy tính về.",
        "03-uzun-anahtar.png": "Một khóa SSH 386 ký tự, đầy đủ. Vân tay bên dưới cho phép kiểm chứng một giá trị mà không cần đọc nó.",
        "09-gunluk.png": "Nhật ký sử dụng: cái gì đã đi đâu và khi nào — nhưng không bao giờ là bản thân giá trị.",
        "05-uretici.png": "Trình tạo, với độ hỗn loạn kết quả được hiển thị tính bằng bit.",
    },
}
