...
G["ja"] = dict(
    nav_label="ガイド",
    title="Sekuvo — ガイド",
    desc="Sekuvoの使い方: 初回起動、自動入力、キーボード、Bluetoothでのコンピューターへの入力、バックアップと転送。",
    h1="ガイド",
    lede="Sekuvoができることすべてを、おそらく必要になる順番で説明します。ここに書かれていることは、アカウントもインターネット接続も必要としません。",
    back="← sekuvo.comに戻る",
    sections=[
        ("start", "初回起動", [
            "<p>初めて開くとき、<strong>マスターパスワード</strong>を設定します。保存するすべての秘密情報はこのパスワードから導出された鍵で暗号化されるため、唯一Sekuvoが復元を手伝えないものです — 続ける前に安全な場所に書き留めてください。</p>",
            "<p>他の端末からの移行ですか？ ようこそ画面で<strong>「バックアップがあります — 復元する」</strong>をタップしてください。それでも先にこの端末のマスターパスワードを設定し、その後ファイル選択画面が自動的に開き、あなたのアイテムが戻ってきます。</p>",
            "<p>あなたの端末に指紋または顔認証センサーがある場合、Sekuvoはセットアップ直後にその使用を提案します。これは利便性のための一層に過ぎません: マスターパスワードは常に機能し、工場出荷時リセットを乗り越えるのもこのパスワードです。</p>",
        ]),
        ("entries", "保存できるもの", [
            "<p>4種類のアイテムが、人々が保管庫に保存するもののほとんどをカバーします:</p>",
            "<ul><li><strong>アカウント / パスワード</strong> — ユーザー名、パスワード、サイトまたはアプリ。</li><li><strong>日常</strong> — 氏名、電話番号、メール、住所。フォームが常に求めるもの。</li><li><strong>カード</strong> — カード番号、有効期限、CVV、IBAN。</li><li><strong>セキュアメモ</strong> — 自由なテキスト、スクリプト、鍵、復旧コード、設定ファイル用のサブタイプ付き。</li></ul>",
        ...
        ("autofill", "自動入力（端末内）", [
            ...
            "<p><strong>有効にするには:</strong> Sekuvo → 設定 → <em>自動入力を有効にする</em> → システムのリストからSekuvoを選択してください。このリストの名前は端末によって異なり、SamsungとAndroid 14以降では<em>パスワード、パスキー、自動入力 → 優先サービス</em>、素のAndroidでは<em>自動入力サービス</em>となります。</p>",
            "<p>その後、どのアプリでもユーザー名、パスワード、カードのフィールドに触れると、対応するアイテムがキーボードの上に表示されます。保管庫がロックされている場合、先にロック解除の手順が表示されます — ロックされている間、システムには一切何も渡されません。</p>",
        ]),
        ("keyboard", "Sekuvoキーボード（端末内）", [
            "<p>キーボード拡張機能は、どのアプリのどのフィールドにも、コピーの手順なしで保存された値を入力します。</p>",
            "<p><strong>有効にするには:</strong> Sekuvo → 設定 → <em>キーボードを有効にする</em> → システムのリストで有効化し、必要なときにキーボード切り替えから「Sekuvoキーボード」を選択してください。</p>",
            "<p>最近使用したアイテムが上部に表示され、🔍はタイトル、ユーザー名、住所を検索します。保管庫がロックされている間、キーボードにはクイックアクセスに設定されたアイテムのみ表示されます — 下記を参照してください。</p>",
        ]),
        ("bluetooth", "Bluetoothでコンピューターに入力", [
            "<p>あなたの端末はBluetoothキーボードとして機能し、コンピューターのカーソル位置に秘密情報を入力できます。コンピューターには何もインストールされず、Windows、macOS、Linuxで動作し、値がクリップボードやネットワークに触れることは決してありません。Android 9以降が必要です。</p>",
            "<h3>手順</h3>",
            "<ol><li>Sekuvoでアイテムを開き、入力したいフィールドの横にある💻アイコンをタップします。<strong>この画面を開いたままにしてください</strong> — 端末はこの画面が開いている間だけキーボードとして認識されます。</li><li>コンピューターで端末を<em>新しいBluetooth機器</em>として追加します: Windowsでは<em>設定 → Bluetoothとデバイス → デバイスを追加 → Bluetooth</em>、macOSでは<em>システム設定 → Bluetooth</em>。</li><li>両側でペアリングコードを確認します。</li><li>端末に戻り、リストからコンピューターを選択し、「接続済み」と表示されるのを待ちます。</li><li><strong>コンピューター側</strong>のキーボードレイアウトを選択してください — 端末側ではありません。キーコードを解釈するのはコンピューターなので、レイアウトを間違えると@ \" ? のような文字が気づかないうちに別の文字に変わってしまいます。</li><li>値を入力したいコンピューター側のフィールドをクリックします。</li><li>端末で<strong>入力</strong>をタップします。3秒のカウントダウンの後 — これはあなたがそのフィールドをクリックする時間を確保するためのものです — 値が1キーずつ入力されます。</li></ol>",
            "<h3>接続されているのに何も入力されない</h3>",
            "<p>ほとんどの場合、原因は同じです: コンピューターが以前にあなたの端末を<em>普通の電話として</em>ペアリングしていたため、キーボードサービス（HID）を有効にしたことがない、というものです。接続自体は成功しますが、キー入力がどこにも届きません。</p>",
            "<ol><li>コンピューター側でペアリングを解除します（<em>デバイスを削除</em>）。</li><li>端末側でも解除します（Bluetooth設定で<em>ペア解除</em>）。両側とも重要です。</li><li>Sekuvoで💻画面を開き、開いたままにします。</li><li><strong>コンピューター側から</strong>始めて再度ペアリングします。</li><li>Windowsで確認するには: <em>デバイスマネージャー → ヒューマンインターフェースデバイス</em>に、今度はBluetooth HID機器が表示されるはずです。</li></ol>",
            "<p>macOSでは、最初の接続時に<strong>キーボード設定アシスタント</strong>が開くことがあり、shiftキーの隣のキーを押すよう求められます。そのウィンドウが閉じられるまで、macOSは何も処理しません — これがMacで「接続済みだが無反応」になる一般的な原因です。</p>",
            "<h3>速度と、その確認方法</h3>",
            "<p>キー入力は1つずつ送信されるため、長い秘密情報には実際に時間がかかります — ダイアログには見積もりが表示されます。3段階の速度が用意されており、「安全」がデフォルトになっているのは意図的です。速度を上げる前に<strong>⏱ 速度テスト</strong>を使ってください: 10個の同一のブロックをスペースで区切って入力し、測定された1秒あたりの文字数を報告します。1つでもブロックが異なる場合、その速度はこのコンピューターでは安全ではありません — 1段階下げてください。</p>",
            "<p>速度テストは診断としても使えます: 何も入力されない場合はペアリングの問題、文字が乱れる場合はレイアウトの問題、入力が途中で止まる場合は速度の問題です。</p>",
            "<p>入力中は進行状況バーと<strong>停止</strong>ボタンが表示されます。送信キューが詰まった場合、Sekuvoは待機時間を増やしながら再試行し、それでも失敗する場合は<em>停止して、どの文字で止まったかを知らせます</em> — 秘密情報が黙って途中まで入力されたままになることはありません。</p>",
        ]),
        ("lock", "ロックの仕組み", [
            "<p>保管庫が開かれている画面が消えた瞬間にロックされます — 折りたたみ式端末を閉じることも含まれます。その瞬間に鍵はメモリから消去されます。アイテムのタイトルはリストに表示され続けることがありますが、何も復号できません。</p>",
            "<p>画面が点いたままアプリを終了しても<em>ロックされません</em>。これは意図的です: アプリ、キーボード、自動入力は1つのセッションを共有しており、そうでなければパスワードを貼り付けるためにブラウザに切り替えるだけで、作業の途中で締め出されてしまいます。</p>",
        ]),
        ("quick", "クイックアクセス — 意図的なトレードオフ", [
            "<p>明示的に「パスワードなしでキーボードから使う」と設定したアイテムは、別のデバイス鍵で暗号化されて2回目の保存が行われ、キーボードが<em>保管庫のロック中でも</em>それらを読み取れるようにします。</p>",
            "<p>このトレードオフは明確に示されています: そのようなアイテムは、マスターパスワードではなく、あなたの端末の画面ロックによって保護されます。パスワードはそこに置かないでください。この設定はデフォルトで無効です。唯一の例外は新規の<strong>日常</strong>アイテムで、氏名や電話番号はまさに手元にすぐ用意しておきたいものであるため、最初から有効になっています — どちらも個別に無効化できます。</p>",
        ]),
        ("backup", "バックアップと復元", [
            "<p>システムバックアップ（Googleバックアップ、端末移行）は意図的に無効化されているため、保管庫が端末から出る方法は、あなたが選ぶ1つだけです: <strong>設定 → 暗号化されたバックアップを作成</strong>。</p>",
            "<p><em>バックアップパスワード</em>を選択します — マスターパスワードとは異なるものにしてください。バックアップパスワードは最終的にコンピューターに入力されるものだからです。結果として1つの<code>.vaultbak</code>ファイルが生成され、Drive、SDカード、USBメモリなど好きな場所に保存できます。</p>",
            "<p>このファイルは端末に依存しません: 独自のソルトと鍵導出パラメータがヘッダーに含まれています。ファイルとバックアップパスワードがあれば、工場出荷時リセット後でも、どの端末でもすべて復元できます。<strong>設定 → バックアップから復元</strong>、または新規インストール時のようこそ画面から復元してください。</p>",
            "<p>復元では<em>既存のものに追加</em>または<em>すべて置き換え</em>を選べます。どちらも何かが書き込まれる前に検証されます — 間違ったパスワードは認証の段階で失敗し、新しいデータが確認されるまで何も削除されません。</p>",
        ]),
        ("transfer", "コンピューターから秘密情報を持ち込む", [
            "<p>長い鍵を端末に手で入力するのは、ミスが起きやすい場面です。Sekuvoはそれらを暗号化された封筒として、2つの方法で転送します: あなたが貼り付けるテキスト、またはあなたがスキャンするQRコードです。</p>",
            "<p><strong>端末上で:</strong> ➕ボタンは<em>QRでインポート</em>と<em>テキストからインポート</em>を提供します。QRインポートはカメラを開きます。画像は端末上でデコードされ、複数画像にわたる転送は収集中の進行状況を表示します。</p>",
            "<h3>コンピューター側のツール</h3>",
            "<p>これらの封筒を作成するツールは1つのHTMLファイルであり、意図的に<strong>ウェブサイトではなくダウンロードファイル</strong>です。sekuvo.comがあなたに秘密情報を尋ねることは決してありません。もしそうするページがあれば、それはフィッシングサイトとまったく同じに見えるはずです。</p>",
            "<ol><li>GitHubでプロジェクトを開き、<strong>Releases</strong>に移動します。</li><li>最新リリースから<code>aktar.html</code>をダウンロードします。</li><li>横に公開されている値でSHA-256を確認します: macOSまたはLinuxでは<code>shasum -a 256 aktar.html</code>、Windowsでは<code>certutil -hashfile aktar.html SHA256</code>。</li><li>ファイルをダブルクリックして開きます。ディスクから実行されます — アドレスバーにはウェブサイトではなく<code>file://</code>が表示されます。</li><li>テキストを貼り付け、転送パスワードを設定すると、テキストまたはQRコードとして封筒が生成されます。</li><li>端末でスキャンまたは貼り付けを行い、同じパスワードを入力して、追加または置き換えを選択します。</li></ol>",
            "<p>すべての暗号化はあなたのブラウザ内、あなたのマシン上で行われます。コマンドライン用の<code>vault-clip.py</code>もあり、クリップボードまたはファイルから同じことを行い、ターミナルにQRコードを描画することもできます。</p>",
            "<h3>ツールを使いたくない場合</h3>",
            "<p>この封筒はオープンなフォーマットであり、Sekuvoでしか作れないものではありません: PBKDF2-HMAC-SHA256（310,000回）、AES-256-GCM、それらが小さなJSONオブジェクトにラップされています。すでに読んだ約30行のコードで自分で作成し、結果を<em>テキストからインポート</em>に貼り付けることができます — アプリはどのツールがそれを作成したか知る術がなく、気にもしません。</p>",
            "<p>作り方はリポジトリにあります: <a href=\"https://github.com/afgover/Vault/blob/HEAD/docs/kendi-zarfini-uret.md\">docs/kendi-zarfini-uret.md</a>（トルコ語で書かれています）。</p>",
        ]),
        ("log", "使用履歴", [
            "<p>Sekuvoは、どのアイテムのどのフィールドが、いつ、どこへ送られたかを記録します: クリップボードへ、Bluetooth経由でコンピューターへ（送信先の端末名付き）、またはキーボードからアプリに入力された場合。コンピューターが侵害されたとき、これは「何がそこに渡ったか、何を変更する必要があるか」に答えます。</p>",
            "<p>この履歴はあなたのアイテムと同じ鍵で暗号化されており、<strong>値そのものが記録されることは決してありません</strong> — イベントの種類、フィールド名、送信先のみです。アイテムを削除すると、その履歴も削除され、設定からすべてを消去できます。</p>",
        ]),
        ("generator", "パスワード生成ツール", [
            "<p>ホーム画面の🎲アイコン、またはアイテム編集中のパスワードフィールドの横からアクセスできます。暗号学的な乱数源を使用し、8～64文字を生成し、文字クラスの選択が可能で、互いに紛らわしい文字を除外でき、結果のエントロピーをビット単位で表示します。</p>",
        ]),
    ],
)
...
G["pcm"] = dict(
    nav_label="Guide",
    title="Sekuvo — Guide",
    desc="How to use Sekuvo: first time you run am, autofill, the keyboard, typing for computer through Bluetooth, backups and transfers.",
    h1="Guide",
    lede="Everything wey Sekuvo dey do, for the order wey you go likely need am. Nothing here need account or internet connection.",
    back="← Go back to sekuvo.com",
    sections=[
        ("start", "First time you dey run am", [
            "<p>When you first open am, you go set <strong>master password</strong>. Every secret wey you save go encrypt with key wey dem derive from am, so na the only thing wey Sekuvo no fit help you recover — write am for safe place before you continue.</p>",
            "<p>You dey come from another phone? Tap <strong>\"I get backup — restore am\"</strong> for the welcome screen. You still go set master password for dis device first, then the file picker go open by itself and your entries go come back.</p>",
            "<p>If your phone get fingerprint or face sensor, Sekuvo go offer to use am right after setup. Na just convenience layer: the master password go always work, and na the same one wey go survive factory reset.</p>",
        ]),
        ("entries", "Wetin you fit save", [
            "<p>Four type of entries dey cover most of the things people dey keep for vault:</p>",
            "<ul><li><strong>Account / Password</strong> — username, password, site or app.</li><li><strong>Everyday</strong> — name, phone, email, address. The things wey forms dey always ask for.</li><li><strong>Card</strong> — card number, expiry date, CVV, IBAN.</li><li><strong>Secure note</strong> — free text, with subtypes for scripts, keys, recovery codes, and config files.</li></ul>",
        ...
        ("autofill", "Autofill (inside the phone)", [
            ...
            "<p><strong>To turn am on:</strong> Sekuvo → Settings → <em>Turn on autofill</em> → choose Sekuvo from the system list. Dis list name different for each device: for Samsung and Android 14+ na <em>Passwords, passkeys &amp; autofill → Preferred service</em>; for pure Android na <em>Autofill service</em>.</p>",
            "<p>After dis, touching username, password, or card field for any app go show your matching entries above the keyboard. If the vault dey lock, the unlock step go show first — while e dey lock, the system no go receive anything at all.</p>",
        ]),
        ("keyboard", "Sekuvo Keyboard (inside the phone)", [
            "<p>The keyboard add-on dey type saved values direct inside any field, for any app, without any copy step.</p>",
            "<p><strong>To turn am on:</strong> Sekuvo → Settings → <em>Turn on the keyboard</em> → turn am on for the system list, then choose \"Sekuvo Keyboard\" from the keyboard switcher any time you need am.</p>",
            "<p>Entries wey you use recently dey stay on top, and 🔍 dey search titles, usernames, and addresses. While the vault dey lock, the keyboard go only show entries wey you mark for quick access — see below.</p>",
        ]),
        ("bluetooth", "Typing for computer through Bluetooth", [
            "<p>Your phone fit act as Bluetooth keyboard and e go type secret for where your computer cursor dey. Nothing no dey install for the computer, e dey work for Windows, macOS, and Linux, and the value no dey ever touch clipboard or network. E need Android 9 or newer.</p>",
            "<h3>Step by step</h3>",
            "<ol><li>For Sekuvo, open the entry and tap the 💻 icon near the field wey you want type. <strong>Keep dis screen open</strong> — the phone go only announce itself as keyboard while dis screen dey open.</li><li>For the computer, add the phone as <em>new Bluetooth device</em>: for Windows na <em>Settings → Bluetooth &amp; devices → Add device → Bluetooth</em>, for macOS na <em>System Settings → Bluetooth</em>.</li><li>Confirm the pairing code for both sides.</li><li>Go back to the phone, choose your computer from the list and wait for \"Connected\" to show.</li><li>Choose the <strong>computer's</strong> keyboard layout — no be the phone own. Na the computer dey interpret the key codes, so if you choose wrong layout, character like @ \" ? go quietly turn to something else.</li><li>Click inside the field for the computer where the value suppose go.</li><li>Tap <strong>Type</strong> for the phone. After three-second countdown — wey dey there so you fit click inside dat field — the value go type key by key.</li></ol>",
            "<h3>E connect but nothing no dey type</h3>",
            "<p>Almost always na the same reason: the computer don pair your phone before <em>as ordinary phone</em>, so e never turn on keyboard service (HID) for am. The connection go succeed but the key strokes no go land anywhere.</p>",
            "<ol><li>Remove the pairing for the computer (<em>Remove device</em>).</li><li>Remove am for the phone too (<em>Forget</em> for Bluetooth settings). Both sides matter.</li><li>Open the 💻 screen for Sekuvo and leave am open.</li><li>Pair again, starting <strong>from the computer</strong>.</li><li>To confirm for Windows: <em>Device Manager → Human Interface Devices</em> suppose now show Bluetooth HID device.</li></ol>",
            "<p>For macOS, the first connection fit open <strong>Keyboard Setup Assistant</strong>, wey go ask you press key beside the shift key. Until you close dat window, macOS no go process anything — na dis dey the usual reason for \"connected but silent\" for Mac.</p>",
            "<h3>Speed, and how you go check am</h3>",
            "<p>Key strokes dey send one by one, so long secret go really take time — the dialog go show estimate. Three speeds dey available, and the fact say Safe na the default no be by mistake. Before you increase am, use the <strong>⏱ Speed test</strong>: e go type ten identical blocks wey space separate and e go report the measured characters per second. If even one block different, dat speed no safe for dis computer — go one step lower.</p>",
            "<p>The speed test fit also serve as diagnosis: nothing wey type mean pairing problem, character wey scatter mean layout problem, typing wey cut mean speed problem.</p>",
            "<p>While e dey type, you go see progress bar and <strong>Stop</strong> button. If the sending queue hang, Sekuvo go try again with waiting time wey dey increase, and if e still fail, e go <em>stop and tell you</em> which character e stop for — no secret go type halfway quietly.</p>",
        ]),
        ("lock", "How the lock dey work", [
            "<p>The vault go lock the moment the screen wey e open on off — closing foldable phone dey count too. For dat moment, dem go wipe the key from memory; entry titles fit still dey show for the list, but nothing no fit decrypt.</p>",
            "<p>Leaving the app while the screen still on <em>no dey</em> lock am. Na so we design am on purpose: the app, the keyboard, and autofill dey share one session, if not, switching go browser to paste password go lock you out for the middle of wetin you dey do.</p>",
        ]),
        ("quick", "Quick access — deliberate trade-off", [
            "<p>Entries wey you explicitly mark as \"use for keyboard without password\" dem go store am second time, encrypted with separate device key, so the keyboard fit read am <em>while the vault dey lock</em>.</p>",
            "<p>The trade-off dey clear: dis kind entries na your phone lock screen dey protect am, no be your master password. No keep passwords for dis one. The mark dey off by default; the only exception na new <strong>Everyday</strong> entries, wey dey start marked because name and phone number na exactly wetin you go want close by — you fit turn off any of dem.</p>",
        ]),
        ("backup", "Backups and restore", [
            "<p>System backup (Google backup, device transfer) dey off on purpose, so na only one way your vault fit comot from the phone, and na the one wey you choose: <strong>Settings → Create encrypted backup</strong>.</p>",
            "<p>You go choose <em>backup password</em> — make am different from your master password, because na the backup password dem go type for computers. Wetin e produce na one <code>.vaultbak</code> file wey you fit save anywhere: Drive, SD card, USB drive.</p>",
            "<p>The file no depend on the device: e get im own salt and key-derivation parameters for the header. The file plus backup password go restore everything for any phone, even after factory reset. Restore am through <strong>Settings → Restore from backup</strong>, or from the welcome screen for new installation.</p>",
            "<p>Restore dey offer <em>add to wetin dey there</em> or <em>replace everything</em>. Dem go verify both before anything write — wrong password go fail for the authentication step, and nothing no go delete until dem confirm the new data.</p>",
        ]),
        ("transfer", "Bringing secrets from computer", [
            "<p>Typing long key with hand for phone na where mistakes dey happen. Sekuvo dey carry dem as encrypted envelope, two ways: text wey you paste, or QR codes wey you scan.</p>",
            "<p><strong>For the phone:</strong> the ➕ button dey offer <em>Import through QR</em> and <em>Import from text</em>. QR import go open the camera; the frames dey decode for the device, and transfers wey get plenty frames go show progress as dem dey collect am.</p>",
            "<h3>The computer-side tool</h3>",
            "<p>The tool wey dey build dis envelopes na one HTML file, and na deliberate say e be <strong>download, no be website</strong>. sekuvo.com no go ever ask you for secret; any page wey do dat go look exactly like phishing site.</p>",
            "<ol><li>Open the project for GitHub and go to <strong>Releases</strong>.</li><li>Download <code>aktar.html</code> from the latest release.</li><li>Check the SHA-256 with the value wey dem publish beside am: <code>shasum -a 256 aktar.html</code> for macOS or Linux, <code>certutil -hashfile aktar.html SHA256</code> for Windows.</li><li>Open the file with double click. E dey run from your disk — the address bar go show <code>file://</code>, no be website.</li><li>Paste your text, set transfer password, and e go produce the envelope as text or as QR codes.</li><li>For the phone, scan or paste am, enter the same password, and choose add or replace.</li></ol>",
            "<p>All the encryption dey happen inside your browser, for your machine. <code>vault-clip.py</code> dey too for command line, wey do the same thing from clipboard or from file and e fit draw QR codes for terminal.</p>",
            "<h3>If you no wan use the tool</h3>",
            "<p>The envelope na open format, e no be something wey only Sekuvo fit create: PBKDF2-HMAC-SHA256 with 310,000 rounds, AES-256-GCM, wrap inside small JSON object. You fit build am yourself with about thirty lines wey you don already read, and paste the result inside <em>Import from text</em> — the app no get way to know which tool produce am, and e no even care.</p>",
            "<p>The recipe dey inside the repository: <a href=\"https://github.com/afgover/Vault/blob/HEAD/docs/kendi-zarfini-uret.md\">docs/kendi-zarfini-uret.md</a> (dem write am for Turkish).</p>",
        ]),
        ("log", "Usage log", [
            "<p>Sekuvo dey record which field of which entry go where and when: to clipboard, to computer through Bluetooth (with the target device name), or dem type am inside app from the keyboard. When computer don compromise, dis one go answer \"wetin land there, wetin I need change\".</p>",
            "<p>The log dey encrypt with the same key as your entries, and <strong>the values no dey ever write inside am</strong> — na only the event type, field name, and the target. Deleting entry go delete im log too, and you fit clear everything from Settings.</p>",
        ]),
        ("generator", "Password generator", [
            "<p>You fit access am from the 🎲 icon for the home screen, or beside the password field while you dey edit entry. E dey use cryptographic randomness source, e go produce 8 to 64 character, e go let you choose character classes, e fit remove characters wey resemble each other, and e go show the entropy wey e produce for bits.</p>",
        ]),
    ],
)
...
CAPTIONS = {
    ...
    "ja": {
        "01-karsilama.png": "ようこそ画面: マスターパスワードを設定するか、既存のバックアップを復元します。",
        "02-liste.png": "アイテムのリスト、検索と種類フィルター付き。",
        "10-ekleme-menusu.png": "追加時: 4種類のアイテムと、2つのインポート方法。",
        "04-detay.png": "1つのアイテム。パスワードは非表示のままで、各フィールドの横のアイコンでコピーまたはコンピューターへの入力ができます。",
        "07-ayarlar.png": "設定 — ここで自動入力とキーボードが有効化されます。",
        "11-duzenleme.png": "アイテムを編集中。クイックアクセスのスイッチは下部にあり、そのコストが横に説明されています。",
        "08-yedekleme.png": "バックアップと復元は設定内で隣り合っています。",
        "06-aktarim.png": "コンピューターから秘密情報を持ち込むウィザード。",
        "03-uzun-anahtar.png": "386文字のSSH鍵、全文表示。下のフィンガープリントで、値を読まずに検証できます。",
        "09-gunluk.png": "使用履歴: 何が、どこへ、いつ送られたか — ただし値そのものは決して記録されません。",
        "05-uretici.png": "生成ツール、結果のエントロピーがビット単位で表示されています。",
    },
    "pcm": {
        "01-karsilama.png": "The welcome screen: set master password, or restore backup wey you already get.",
        "02-liste.png": "The entries list, with search and type filters.",
        "10-ekleme-menusu.png": "When you dey add: four types of entries, plus the two import paths.",
        "04-detay.png": "One entry. The password dey stay hidden; the icons beside each field go copy am or type am for computer.",
        "07-ayarlar.png": "Settings — na here autofill and keyboard dey turn on.",
        "11-duzenleme.png": "Editing entry. The quick-access switch dey sit down, im cost dey explain beside am.",
        "08-yedekleme.png": "Backup and restore dey stay together for Settings.",
        "06-aktarim.png": "The wizard for bringing secrets from computer.",
        "03-uzun-anahtar.png": "One 386-character SSH key, full. The fingerprint below am dey let you verify value without you read am.",
        "09-gunluk.png": "The usage log: wetin go where and when — but never the value itself.",
        "05-uretici.png": "The generator, with the entropy wey e produce showing for bits.",
    },
}
